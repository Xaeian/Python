...
Xaeian wraps stdlib boilerplate into intent-level APIs.
You read what code does, not how.
In real projects this cuts **~35%** of code volume.
...
- Explicit over implicit: `PATH.resolve` auto-joins with CWD, `db.insert` auto-serializes
  dicts to JSON and ISO strings to datetime.
  Convenient in 95% of cases, surprising in the rest.
  Deliberate, and switchable where it gets in the way (`auto_resolve=False`).
- Performance: an extra abstraction layer costs.
  Path resolution, auto-serialization and placeholder conversion run on every call.
  Negligible for APIs and tools, not for tight loops over millions of rows.

Type safety comes from Pydantic models and type hints on the application layer.
Xaeian is the plumbing underneath.
Each layer does one thing and stays out of the way.
It fits best when you own the stack end-to-end and value compact code over configurability.
