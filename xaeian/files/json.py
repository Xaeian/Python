...
class JSON:
  ...
  def smart(
    ...
    Numeric arrays wrap every `array_wrap` values, 2D numeric arrays get one row per line,
    and `compact_dict` packs flat dicts several entries per line instead of one key per line.
    """
    def _dumps(v, **kw):
    ...
    def format_numeric_row(arr, base_indent) -> str:
    ...
    def format_2d_numeric(arr, depth) -> str:
      pad = " " * (depth * indent)
      pad_inner = " " * ((depth + 1) * indent)
      rows = [format_numeric_row(row, pad_inner) for row in arr]
      return "[\n" + pad_inner + (",\n" + pad_inner).join(rows) + "\n" + pad + "]"
    def format_flat_dict(d, depth) -> str:
      pad = " " * (depth * indent)
      pad_inner = " " * ((depth + 1) * indent)
