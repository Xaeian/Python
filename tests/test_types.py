...
assert_type(FILE.save("a.txt", "x"), Any) # no return annotation
...
assert_type(INI.load("a.ini"), dict[Any, Any])
...
assert_type(st.decode(b""), list[dict[Any, Any]]|dict[Any, Any])
...
assert_type(log.inf("x"), Any) # the message methods carry no annotations
assert_type(Print().level, int)

rows: list[dict[str, Any]] = []
assert_type(tbl.where(rows, lambda r: True), list[dict[str, Any]])
assert_type(tbl.first(rows, lambda r: True), dict[Any, Any]|None)
...
assert_type(db.get_dict("select 1"), dict[Any, Any]|None)
assert_type(db.get_dicts("select 1"), list[dict[Any, Any]])
assert_type(db.get_value("select 1"), Any)
assert_type(db.insert("t", {"name": "Jan"}), int|Any) # widened by the `returning` argument
assert_type(db.find_one("t", name="Jan"), dict[Any, Any]|None)
...
async def use_async() -> None:
  ...
  assert_type(await adb.get_dicts("select 1"), list[dict[Any, Any]])
  assert_type(await adb.insert("t", {"name": "Ola"}), int|Any)
  assert_type(await adb.find_one("t", name="Ola"), dict[Any, Any]|None)
  assert_type(adb.transaction(), Any) # abstract, no return annotation
'''
